...
export default async function getPlayerDetails(gameID) {
  try {
    const response = await axios.get(
      `https://stats.warbrokers.io/players/i/${gameID}`
    );
    ...
    if (response.status == 200) {
      ...
      const level = $(
        "body > div.mainContent > div.page-banner-container > div > span"
      )
        .text()
        .replace(/Lvl /g, "");
      const levelProgress = Number(
        $(
          "body > div.mainContent > div.feature > div > div > div.progressContainer > div.progress > div > span"
        )
          .text()
          .replace(/%/g, "")
      );
      const levelProgressBar = new stringProgress(
        100,
        levelProgress
      ).filledBar()["bar"];
      const squad = $(
        "body > div.mainContent > div.page-banner-container > div"
      )
        .text()
        .replace(gameName, "")
        .replace(`Lvl ${level}`, "")
      ...
      const kills = Number(
        $(
          "#player-details-summary-grid > div:nth-child(2) > div.player-details-number-box-value"
        )
          .text()
          .replace(/,/g, "")
          .replace(/\n/g, "")
      ...
      const deaths = Number(
        $(
          "#player-details-summary-grid > div:nth-child(3) > div.player-details-number-box-value"
        )
          .text()
          .replace(/,/g, "")
          .replace(/\n/g, "")
