...
(async () => {
  ...
  for (const file of commandFiles) {
    ...
    if ("data" in command && "execute" in command) {
      ...
      console.log(
        `registerSlashCommands : the command at ${filePath} is missing a required "data" or "execute" property`
      );
    }
  }
  ...
  client.on(
    "interactionCreate",
    async (interaction) => await InteractionCreateHandler(client, interaction)
  );
  client.on(
    "messageCreate",
    async (message) => await MessageCreateHandler(message)
  );
  client.on(
    "channelDelete",
    async (channel) => await ChannelDeleteHandler(channel)
  );
  client.on(
    "threadDelete",
    async (thread) => await ThreadDeleteHanhler(thread)
  );
  ...
  client.on("ready", async () => {
    ...
    const app = server.listen(server.get("port"), server.get("host"), () => {
      ...
      console.log(
        `Server : client is on (took ${
          readableTime(Math.round(finishTime - startTime))["string"]
        })`
      );
    });
    server.get("/", (_, res) => {
      res
        ...
        .json(
          `Service is up : ${
            readableTime(Math.round(performance.now()))["string"]
          }`
        );
    });
