  ...
  try {
    ...
    const title = await (async () => {
      if (oldMessageObj.messages.length === 2) {
        ...
        const chatTitle = (await requestChat(oldMessages)).choices[0].message
          .content;
    ...
    const gptReply = (await requestChat(oldMessageObj.messages)).choices[0]
      .message;
    ...
    await previousMessage.edit(
      "Can't connect to ChatGPT, please try again later."
    );
  }
}
