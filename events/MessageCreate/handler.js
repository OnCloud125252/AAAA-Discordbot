...
export default async function MessageCreateHandler(message) {
  ...
  switch (true) {
    case content.toLowerCase() === "kd" ||
      (content.startsWith("A ") &&
        (content.toLowerCase() === "a ping" ||
          content.toLowerCase() === "a botinfo")): {
      warning(message);
      break;
    }

    case message.channel.type === ChannelType.PublicThread &&
      message.channel.parent?.type === ChannelType.GuildForum &&
      message.channel.parentId === guildObj.chatChannelID: {
      chat(message);
      break;
    }
