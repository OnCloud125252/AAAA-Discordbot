...
export default {
    ...
    async execute(client, interaction, version) {
        var validUser = interaction.options.getMentionable("target")?.user?.id ?? false;
        ...
        if (validUser) {
            const authorID = interaction.options.getMentionable("target")?.user?.id ?? interaction.user.id;
