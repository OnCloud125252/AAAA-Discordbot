...
export default {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((subcommand) =>
      subcommand.setName("delete").setDescription("Delete current chat")
    )
    ...
    try {
      ...
      if (!guildObj.chatChannelID) {
        await await interaction.reply(
          "Please config the chat channel first using </config chat:1114143236789915768> command."
        );
        return;
      }
    }
    ...
    switch (interaction.options.getSubcommand()) {
      case "create": {
                ...
                fields: [
                  {
                    name: "Important:",
                    value:
                      "All your conversation will be stored in database until you delete this chat.",
                    inline: false
                  },
                  {
                    name: `System ${
                    ...
                    value:
                      interaction.options.getString("prompt") ?? defaultPrompt,
                    inline: false
                  },
                  {
                    name: "Module:",
        ...
        try {
          ...
          const oldMessage = (await updateChat(newChatID, null, [chatSetup]))
            .messages;
          ...
          await interaction.editReply(
            `Successfully created new chat at <#${newChatID}>.`
          );
        }
        catch (error) {
          console.error(error);
          ...
          await interaction.reply(
            "Can't create new chat, please try again later."
          );
        }
        break;
      }
      case "delete": {
        if (
          ...
          interaction.channel.parentId === guildObj.chatChannelID
        ) {
