...
export default {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((subcommand) =>
      subcommand.setName("reset").setDescription("Reset all config")
    )
      ...
      case "chat": {
        ...
        try {
          ...
          await interaction.reply(
            `Successfully register <#${channelObj.id}> for ChatBot in this server.`
          );
        }
        catch (error) {
          await interaction.reply(
            "Can't config chat channel, please try again later."
          );
        }
        break;
      }
