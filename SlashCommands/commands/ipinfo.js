...
export default {
    ...
    async execute(client, interaction, version) {
        ...
        const validIpAddress = /^((25[0-5]|(2[0-4]|1\d|[1-9]|)\d)\.?\b){4}$/.test(ipAddress);
        ...
        if (validIpAddress) {
            ...
            if (ipinfoResponse.data.bogon) {
                ...
                const mapResponse = await axios.get("https://maptoolkit.p.rapidapi.com/staticmap", {
                    params: {
                ...
                const imageMessage = await client.channels.cache.get("874654634533343232").send({
                    files: [imageBuffer]
                });
